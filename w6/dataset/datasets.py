...
from dataset.frame import ActionSpotDataset, ActionSpotVideoDataset, CustomDataset
...
def get_datasets(args):
    classes = load_classes(os.path.join('/ghome/c5mcv04/w6/spotting/data', args.dataset, 'class.txt'))
    ...
    train_data = ActionSpotDataset(
        classes, os.path.join('/ghome/c5mcv04/w6/spotting/data', args.dataset, 'train.json'),
        args.frame_dir, args.store_dir, args.store_mode, args.clip_len, dataset_len, **dataset_kwargs)
    train_data.print_info()

    val_data = ActionSpotDataset(
        classes, os.path.join('/ghome/c5mcv04/w6/spotting/data', args.dataset, 'val.json'),
        args.frame_dir, args.store_dir, args.store_mode, args.clip_len, dataset_len // 4, **dataset_kwargs)
    val_data.print_info()     

    val_data_video = ActionSpotVideoDataset(classes, os.path.join('/ghome/c5mcv04/w6/spotting/data', args.dataset, 'val.json'),
        args.frame_dir, args.clip_len, **dataset_kwargs)
    val_data_video.print_info()

    dataset_kwargs['overlap'] = 0

    test_data = ActionSpotVideoDataset(classes, os.path.join('/ghome/c5mcv04/w6/spotting/data', args.dataset, 'test.json'),
        args.frame_dir, args.clip_len, **dataset_kwargs)
    test_data.print_info()
        
    return classes, train_data, val_data, val_data_video, test_data