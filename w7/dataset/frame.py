...
class CustomDataset(Dataset):
    def __init__(
            self,
            classes,                # Diccionario de clases
            game_file,              # Ruta del archivo de etiquetas (JSON)
            frame_dir,              # Directorio de los frames del video
            store_dir,              # Directorio donde almacenar los clips generados
            store_mode,             # Modo de almacenamiento ('store' o 'load')
            clip_len,               # Número de frames por clip
            dataset_len,            # Número total de clips
            stride=1,               # Paso entre frames
            overlap=1,              # Superposición entre clips
            pad_len=0,              # Longitud de relleno para los clips
            dataset='soccernetball',# Nombre del dataset
            labels_dir=None,        # Directorio de las etiquetas (opcional)
            task='classification',  # Tipo de tarea ('classification' o 'spotting')
    ):
        self._src_file = game_file
        self._games = self._load_json(game_file)
        self._class_dict = classes
        self._video_idxs = {x['video']: i for i, x in enumerate(self._games)}
        self._dataset = dataset
        self._store_dir = store_dir
        ...
        self._labels_dir = labels_dir
        self._task = task
        self._dataset_len = dataset_len if dataset_len is not None else len(self._games)
        
        # Verifica que el tipo de tarea sea válido
        assert task in ['classification', 'spotting']
        
        # Inicializa el lector de frames
        self._frame_reader = FrameReader(frame_dir, dataset=dataset)
        
        # Almacena o carga los clips según el modo
        if store_mode == 'store':
            self._store_clips()
        elif store_mode == 'load':
            self._load_clips()
    ...
    def _store_clips(self):
        """
        Almacena los clips extraídos en el directorio especificado.
        """
        self._frame_paths = []
        self._labels_store = []
        
        for video in self._games:
            video_len = int(video['num_frames'])
            labels_file = self._load_json(os.path.join(self._labels_dir, f"{video['video']}/Labels-ball.json"))['annotations']
            ...
            for base_idx in range(-self._pad_len * self._stride, max(0, video_len - 1 + (2 * self._pad_len - self._clip_len) * self._stride), self._clip_sampling_step):
                ...
                for event in labels_file:
                    event_half = int(event['gameTime'][0])
                    if event_half == video_half:
                        event_frame = int(event['position']) / 1000 * FPS_SN  # convertir a frames
                        label_idx = (event_frame - base_idx) // self._stride
                        if 0 <= label_idx < self._clip_len:
                            label = self._class_dict[event['label']]
                            labels.append({'label': label, 'label_idx': label_idx})
                
                if frames_paths[1] != -1:  # Si hay frames disponibles
                    self._frame_paths.append(frames_paths)
                    self._labels_store.append(labels)
        
        store_path = os.path.join(self._store_dir, f"LEN{self._clip_len}SPLIT{self._split}")
        if not os.path.exists(store_path):
            os.makedirs(store_path)
        
        # Guardar los clips y etiquetas
        with open(store_path + '/frame_paths.pkl', 'wb') as f:
            pickle.dump(self._frame_paths, f)
        with open(store_path + '/labels.pkl', 'wb') as f:
            pickle.dump(self._labels_store, f)
        print(f"Stored clips to {store_path}")
    
    def _load_clips(self):
        """
        Carga los clips desde los archivos almacenados.
        """
        store_path = os.path.join(self._store_dir, f"LEN{self._clip_len}SPLIT{self._split}")
        with open(store_path + '/frame_paths.pkl', 'rb') as f:
            self._frame_paths = pickle.load(f)
        with open(store_path + '/labels.pkl', 'rb') as f:
            self._labels_store = pickle.load(f)
        print(f"Loaded clips from {store_path}")
    
    def __getitem__(self, idx):
        """
        Devuelve un clip y sus etiquetas.
        """
        frames_path = self._frame_paths[idx]
        dict_label = self._labels_store[idx]
        frames = self._frame_reader.load_frames(frames_path, pad=True, stride=self._stride)
        ...
        return {'frame': frames, 'contains_event': int(np.sum(labels) > 0), 'label': labels}
