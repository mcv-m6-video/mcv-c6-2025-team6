...
def main(args):
    ...
    if args.store_to_wandb:
        wandb.init(project="ball-action-detection-w7", entity="c6mcv06", name=f"{os.path.basename(ckpt_dir)}")
    ...
    if not args.only_test:
        ...
        best_map = 0
        epoch = 0
        epochs_without_improvement_loss = 0
        epochs_without_improvement_map = 0
        ...
        for epoch in range(epoch, num_epochs):
            ...
            if epoch >= 0:
                map_score, ap_score = evaluate(model, val_data_video, nms_window = 5)
                map12 = map_score*100
                val_map = np.mean(ap_score[:-2])
                map10 = val_map*100
                # Printing info epoch
                print(f'[Epoch {epoch}] Train loss: {train_loss:0.5f} Val loss: {val_loss:0.5f}, mAP-12: {map12}, mAP-10: {map10}')
                if args.store_to_wandb:
                    wandb.log({"mAP-12": map12,
                            "mAP-10": map10,
                            "epoch": epoch + 1})
            else:
                # Printing info epoch
            ...
            better_loss = False
            if val_loss < best_loss - args.early_stop_delta:
                best_loss = val_loss
                better_loss = True
                epochs_without_improvement_loss = 0
            else:
                epochs_without_improvement_loss += 1
            ...
            if val_map > best_map:
                ...
                epochs_without_improvement_map += 1
            ...
            if better_loss:
                print('New best loss epoch!')

            if better_map:
                print('New best mAP epoch!')

            if (epochs_without_improvement_loss >= args.early_stop_patience) and (epochs_without_improvement_map >= args.early_stop_patience):
                print(f'Early stopping triggered after {epochs_without_improvement_loss} epochs with no loss improvement \
                      and {epochs_without_improvement_map} epochs with no mAP improvement.')
                break

            losses.append({
                'epoch': epoch, 'train': train_loss, 'val': val_loss, 'mAP-12': map12, 'mAP-10': map10
            })
            ...
            if args.save_dir is not None:
                ...
                if better_map:
                    torch.save( model.state_dict(), os.path.join(ckpt_dir, 'checkpoint_best_map.pt') )
        
            torch.save(model.state_dict(), os.path.join(ckpt_dir, 'checkpoint_last.pt'))

    print('===== START INFERENCE OVER BEST MODEL =====')
    model.load(torch.load(os.path.join(ckpt_dir, 'checkpoint_best_map.pt')))
