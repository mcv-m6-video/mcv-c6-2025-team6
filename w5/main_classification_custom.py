...
from model.model_classification_custom import Model
...
def main(args):
    ...
    #model.load(torch.load(os.path.join('/ghome/c5mcv04/w5/results/effnet_b1/_exp1/checkpoints', 'checkpoint_best.pt')))
    ...
    if not args.only_test:
        ...
        for epoch in range(epoch, num_epochs):
            ...
            better = False
            delta = 0.01 
            if val_loss < best_criterion - delta:
    ...
    avg_table = [["Average 12", f"{np.mean(ap_score)*100:.2f}"],["Average 10", f"{np.mean(ap_score[:-2])*100:.2f}"]]
